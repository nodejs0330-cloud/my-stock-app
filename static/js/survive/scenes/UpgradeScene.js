class UpgradeScene extends Phaser.Scene {
            ...
            create() {
                ...
                console.log("[UpgradeScene] DB 최신 강합 수치 조회 시도");
                fetch('/api/survive/user_data')
                    .then(res => res.json())
                    .then(data => {
                        console.log("[UpgradeScene] 수신된 최신 유저 데이터:", data);
                        if (data && data.success) {
                            ...
                            if (data.upgrades) {
                                ...
                                keyMap.forEach(upperKey => {
                                    ...
                                    let val = data.upgrades[lowerKey] !== undefined ? data.upgrades[lowerKey] : data.upgrades[upperKey];
                                    val = Number(val) || 0;
                            ...
                            console.error("❌ [UpgradeScene] DB 조회 실패:", data);
                        }
                        this.renderUpgradeUI();
                    })
                    .catch(err => {
                        console.error("❌ [UpgradeScene Error] API 요청 네트워크 에러:", err);
                        this.renderUpgradeUI();
                    });
            }
